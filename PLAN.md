...
| **M9** | **Workflows.** Engine + seed the 4 workflows (SPEC §6): submit → start level, outcome buttons, Override, Remove from Workflow (unsubmit), field updates per level, timeline with comments, "My Assigned" queue, bulk move | drive a Project from Surveying to Complete on the phone | each workflow's transitions tested |
...
| **M12** | **Dashboards + admin.** Per-module dashboard (My Assigned, My Tasks, Recently Modified); admin screens: users ↔ groups, permission matrix, utility lists, automations | manage users without me | admins can do everything SPEC §7 gives them |
